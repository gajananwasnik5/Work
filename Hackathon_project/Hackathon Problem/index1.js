...
const airportSearch = [
  ...
  let SearchFlight = function() {
    ...
    /* Validating Form after Submit Button */
  ...
  function searching() {
    var input, mobile, filter, a, txtValue;
    input=document.getElementById("myInput");
    filter = input.value.toUpperCase();
    mobile = document.querySelectorAll(".product");
    for(let i=0; i<mobile.length;i++)
    {
        a=mobile[i].getElementsByClassName("title")[0];
        txtValue = a.textContent || a.innerText;
        if (txtValue.toUpperCase().indexOf(filter) > -1) {
            mobile[i].style.display = "initial";
        } else {
